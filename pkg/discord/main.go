...
func UserReact(s *discordgo.Session, m *discordgo.MessageReactionAdd) {
	...
	if channel, err := s.Channel(m.ChannelID); err == nil && channel.Type == discordgo.ChannelTypeGuildText && channel.ParentID == brigade.ActiveProjectCategoryID && m.Emoji.Name == brigade.IssueEmoji {
		if msg, err := s.ChannelMessage(m.ChannelID, m.MessageID); err != nil {
			...
			errorMessage := github.CreateIssue(msg.Content, channel.Name, brigade.GuildID)
			if errorMessage != nil {
...
func onboard(data CommandData) []string {
	...
	var brigade models.Brigade
	err := db.Where("guild_id = ?", data.GuildID).First(&brigade).Error
	if err != nil {
		fmt.Println("error fetching brigade,", err)
		return []string{"Failed to get brigade for onboarding. Try again later."}
	}
	return onboardGroup(data, brigade.OnboardingRole)
}
...
func onboardAll(data CommandData) []string {
	...
	return onboardGroup(data, brigade.OnboardingRole, brigade.NewUserRole)
}
	...
	for _, member := range guild.Members {
		for _, role := range r {
			if contains(member.Roles, role) {
				...
				if err = data.Session.GuildMemberRoleAdd(guildID, member.User.ID, brigade.MemberRole); err != nil {
					fmt.Println("error adding guild role,", err)
...
func getAgenda(data CommandData) []string {
	...
	if err != nil {
		fmt.Println("error fetching brigade,", err)
		return []string{"Failed to get brigade to fetch agenda. Try again later."}
	}
	var message string
	message, data.Errors = gdrive.FetchAgenda(brigade.ID)
...
func joinProject(data CommandData) []string {
	...
	if roles, err := data.Session.GuildRoles(data.GuildID); err != nil {
		...
		if channel, err := data.Session.UserChannelCreate(data.Author.ID); err != nil {
			...
			github.AddUserToTeamWaitlist(data.Author.ID, brigade.GithubOrganization, projectName)
		}
	}
	return nil
}
	...
	for _, user := range users {
		...
		if roles, err := data.Session.GuildRoles(data.GuildID); err != nil {
			...
			if channel, err := data.Session.UserChannelCreate(data.Author.ID); err != nil {
				...
				github.AddUserToChampionWaitlist(discordUser.ID, brigade.GithubOrganization, projectName)
			}
		}
	}
	return data.Errors
...
func trackFile(data CommandData) []string {
	...
	if err != nil {
		fmt.Println("error fetching brigade,", err)
		return []string{"Failed to get brigade to track file. Try again later."}
	}
	fileName := strings.ToLower(data.Args[0])
	link := data.Args[1]
	file, err := fetchFile(data)
	if file != nil {
		return []string{"A file with the name **" + fileName + "** is already tracked: " + file.URL}
	} else if err != nil {
...
func maintainProject(data CommandData) []string {
	...
	if err != nil {
		fmt.Println("error fetching brigade,", err)
		return []string{"Failed to get brigade to move project to maintenance. Try again later."}
	}
	projectName := data.Args[0]
	...
	editData := discordgo.ChannelEdit{
		ParentID: brigade.InactiveProjectCategoryID,
	}
